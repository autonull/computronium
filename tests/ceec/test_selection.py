...
class TestHardConstraints:
    def test_complete_candidate_passes(self, store, scope, link_belief):
    ...
    def test_missing_controls_rejected(self, store, scope, link_belief):
        bootstrap_belief(store, scope, link_belief)
        experiment = make_experiment(
...
class TestDecide:
    def test_decision_recorded_with_selection(self, store, scope, link_belief):
        bootstrap_belief(store, scope, link_belief, "B-H1", "G-1")
        experiment = make_experiment(
    ...
    def test_scoring_deterministic_under_fixed_state(self, store, scope, link_belief):
        bootstrap_belief(store, scope, link_belief, "B-H1", "G-1")
        experiment = make_experiment(
            experiment_id="X-DET",
            belief_ids=["B-H1"],
    ...
    def test_state_hash_changes_with_state(self, store, scope, link_belief):
        bootstrap_belief(store, scope, link_belief)
    ...
    def test_override_without_rationale_rejected(self, store, scope, link_belief):
        bootstrap_belief(store, scope, link_belief)
        experiment = make_experiment(
            design={
                "seed_plan": "3",
