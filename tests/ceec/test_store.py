...
class TestBeliefRevisions:
    def test_revision_history_preserved(self, store, scope, evidence):
        belief_id = link_belief(store, scope, evidence)
        store.update_belief(
            belief_id,
    ...
    def test_revision_requires_rationale(self, store, scope, evidence):
        link_belief(store, scope, evidence)
    ...
    def test_open_revision_requires_evidence(self, store, scope):
    ...
    def test_gated_status_via_update_belief_refused(self, store, scope, evidence):
        link_belief(store, scope, evidence)
...
class TestAppendOnly:
    ...
    def test_revision_chain_intact_after_status_changes(self, store, scope, evidence):
        belief_id = link_belief(store, scope, evidence)
        gate = store.record_gate_outcome(
...
class TestReferentialIntegrity:
    ...
    def test_status_change_unknown_gate_ref(self, store, scope, evidence):
        belief_id = link_belief(store, scope, evidence)
        with pytest.raises(StoreError, match="missing"):
