"""Case tests for DiffusionEqprop algorithm."""

import torch

from computronium.algorithms.diffusion_eqprop import make_case


def test_make_case_returns_case():
    """Test that make_case returns a Case object with expected fields."""
...
def test_make_case_config_structure():
    ...
    assert "lr" in case.config
    assert "output_dim" in case.config
    assert "seed" in case.config
    assert case.config["lr"] == 1e-3
    assert case.config["output_dim"] == 4
