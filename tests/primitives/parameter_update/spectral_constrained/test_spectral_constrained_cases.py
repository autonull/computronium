"""Case factory tests for Spectral Constrained primitive."""

import torch

from computronium.primitives.parameter_update.spectral_constrained.cases import (
    Case,
    make_case,
)
...
def test_make_case_config_structure():
    ...
    assert "spectral_norm" in case.config
    assert "momentum" in case.config
    assert "ortho_steps" in case.config
