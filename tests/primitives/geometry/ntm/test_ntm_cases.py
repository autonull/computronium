"""Case factory tests for NTM Geometry primitive."""

import torch

from computronium.primitives.geometry.ntm.cases import (
    Case,
    make_case,
)
...
def test_make_case_config_structure():
    ...
    assert "mem_slots" in case.config
    assert "mem_width" in case.config
    assert "beta_init" in case.config
