"""Structural equivalence tests for Feedforward DAG Geometry primitive.
...
def test_feedforward_dag_geometry_factory_deterministic():
    """Two FeedforwardGeometry instances with same config and seed have bitwise-equal parameters."""
    ...
    geom1 = FeedforwardGeometry(config)

    # Create second geometry with same seed
    torch.manual_seed(42)
    geom2 = FeedforwardGeometry(config)
...
def test_feedforward_dag_geometry_different_seeds_produce_different_params():
