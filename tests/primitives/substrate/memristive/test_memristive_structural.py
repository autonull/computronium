"""Structural equivalence tests for Memristive Substrate primitive.

Tests factory determinism and spec round-trip for structural primitives.
"""

from computronium.ontology.substrate import MemristiveSubstrate, SubstrateConfig
from computronium.ontology.substrate.spec import SubstrateSpec, make_substrate


def test_memristive_substrate_factory_deterministic():
    """Two substrate instances with same spec have identical config."""
    config = SubstrateConfig.memristive(device="cpu")
    ...
    assert isinstance(substrate1, MemristiveSubstrate)
    assert isinstance(substrate2, MemristiveSubstrate)

    # Configs should be identical
    assert substrate1.config == substrate2.config


def test_memristive_substrate_spec_round_trip():
