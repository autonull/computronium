"""Test cases for Noisy Substrate primitive."""
...
from computronium.primitives.substrate.noisy import make_case, make_case_high_noise
...
def test_make_case_high_noise():
    """Test make_case_high_noise with custom noise level."""
    case = make_case_high_noise(device="cpu", seed=0, noise_level=0.5)

    assert case.spec.noise_model.level == 0.5
    assert case.config["noise_level"] == 0.5
