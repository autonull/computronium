...
| §3.4 retention ratios; settle budget; paired-replay noise scaling | 4.2–5.5× / 19–40× / 940–2600×; settle 238–344 steps | `uv run python scripts/probes/x_sta_001.py && uv run python scripts/probes/x_sta_002.py` (settling sweep + retention/noise ratios; verified in the fresh-clone dry run, 2026-09-11) | X-STA-001, X-STA-002 (E-000028) |
| §3.3 role-split vs parents; FF×Muon collapse | per-seed wins; collapse at width 32 | `uv run python packages/computronium-lab/examples/mechanism_recipes_demo.py` (one-step loss arm); the width-32 collapse is a recipe-level boundary (`when_not` in `computronium_lab/recipes.py`), not re-measured per run | X-USU-001 |
