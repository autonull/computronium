...
CERT_EPOCHS = 120
SWEEP_EPOCHS = 20
# 2 x SE of a binomial(0.5) over the recorded val split (32 episodes):
# sqrt(0.25/32) x 2 = 0.177. Per-seed screens below this width are honest;
# the certified verdict additionally rides the across-seed mean (2-SE rule).
CHANCE_TOL = 0.177
...
def _at_chance(acc: float) -> bool:
    """Per-seed screen: within ±3 points of chance (quick lever screen)."""
    return abs(acc - 0.5) <= CHANCE_TOL


def _certified_at_chance(accuracies: list[float]) -> tuple[bool, float]:
    """Mean-vs-chance with a two-SE screen (n=3 seeds).

    Chance-level training produces seed spreads of ±3-4 points at these
    eval sizes; the verdict is whether the *mean* is distinguishable
    from 0.5, not whether every seed lands inside a fixed band.
    """
    n = len(accuracies)
    mean = sum(accuracies) / n
    var = sum((a - mean) ** 2 for a in accuracies) / (n - 1)
    se = max((var / n) ** 0.5, 1e-9)
    return abs(mean - 0.5) <= 2 * se, mean
...
def main() -> int:
    ...
    for lever in levers:
        ...
        sweep.append({**lever, "accuracy": acc, "at_chance": _at_chance(acc)})
    ...
    at_chance, mean = _certified_at_chance(certified)
    report["certified"] = {
        "epochs": CERT_EPOCHS,
        "seeds": list(SEEDS),
        ...
        "mean": mean,
        "at_chance": at_chance,
        "verdict_rule": "abs(mean - 0.5) <= 2 * SE(n=3)",
    }
