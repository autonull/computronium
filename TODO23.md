...
**Status:** IN PROGRESS (2026-09-13). Phases 1–5 COMPLETE; integration validation: 5-problem-class loop ✓, external quickstart ✓; validation campaigns + CEEC ledger audit + §6 belief promotion + ontology-NTM row + legacy-row re-measurement + **sequence-ψ statistics A/B + NCA state-prediction tier shipped and tested**. Remaining: PyPI publishing (external).
**Created:** 2026-09-11
...
### Session 2026-09-13 — Sequence-ψ Statistics A/B + NCA State-Prediction Tier (final blockers redeemed)
- [x] **`psi_step` knob (the #3 slice)** — `adapt(..., psi_step="final" | "every_timestep")` (default = the recorded final-step contract): `"every_timestep"` engages the P-axis at every sequence step, so the primitive's OWN decayed sufficient statistics (gram/cross) carry across the recurrence — no re-implemented law, single source preserved. `Lab.adapt` threads the knob. **A/B campaign** (`scripts/probes/sequence_psi_stats_ab.py`): trained ntm_sequence + 20 episodes on the task's own stream, 3 seeds — final 0.906/0.867/0.914 vs every_timestep 0.953/0.883/0.883; paired t **mean +0.011, p=0.68 — no significant difference**; `"final"` stays the default (simpler, step contract preserved); negative result recorded. Lock: `test_psi_step_every_timestep_variant` (θ bitwise, ψ updated, bounded).
- [x] **NCA state-prediction tier — the NCA row's recorded blocker redeemed** — `computronium_lab.state_prediction`:
  - **Task finding (recorded)**: the ONE-step hidden-teacher map is degenerate — small tanh deltas mean next≈states, so trained AND permuted-control both hit cell_acc 1.0 (identity prior). The honest task is a **k=3 rollout** target: BPTT through `geometry.rollout(grad=True)` with rollout-target MSE; k=3/delta_scale 0.5 separates trained (0.964/0.999/0.965) from permuted control (~0.61; chance floor 0.25).
  - `grid_transition_task(seed, steps=3)` — fixed random CA teacher per seed (deterministic); `train_state_prediction(system, task, epochs=300, lr=0.02, label_shuffle=...)` — direct autograd, credit axis bypassed (same honest framing as `train_sequence`); `state_prediction_campaign` — §6 gates (BenchmarkReproduction + matched control + DeployabilityCheck; StabilityCertificate recorded as None, the direct path bypasses the classification guard convention), one `validation_campaign` ledger artifact + decision, negative results stored.
  - **`nca_predictor` recipe + catalog row** — W8.1 construction (shared cell MLP, label-free regime), `build_nca_predictor` via `compose_system_from_configs`, `_nca_predictor_config` screened by `SystemConfig.validate()`; Pareto: accuracy **0.96** measured (mean 0.976 @300ep, 3 seeds; control 0.66 matched), latency/memory estimates; provenance carries the one-step degeneracy boundary and the not-wired W8.1 growth/distill-with-labels regime verbatim. Corpus-consistent features (bptt/euclid, exact_gradient, depth 1).
  - **Wiring**: `Lab.train_state_prediction`; exports `StatePredictionResult`, `StatePredictionCampaignReport`, `grid_transition_task`, `state_prediction_campaign`, `train_state_prediction` at `computronium_lab` root. ONNX export of the NCA graph works (tracer warnings on the dim check, same convention as NTM's variable-shape warning).
- [~] **Z3-controller campaign — unchanged verdict** (ontology-blocked: needs a Z3 composition primitive; §11 forbids new axes). **NLP/neuromorphic tiers — unchanged verdict** (real datasets). **PyPI — external.** With this session, **everything completable in-repo under §11 is done.**

**Gates:** lab suite **91 passed** (~95s; test_state_prediction 6 passed); D22 demo + gallery lock green (nca_predictor filtered/dominated — no re-pin needed); ruff clean; pyright strict 0 on all touched modules (2 pre-existing legacy errors in `ecosystem.py` unchanged).

**Notes for future work:**
- `psi_step="every_timestep"` costs T× ψ steps per episode (ridge O(d²), negligible) and changes `trace_steps` growth — never mix variants in one adaptation run; the default keeps all recorded behavior byte-identical.
- The one-step degeneracy finding generalizes: any hidden-teacher task with near-identity dynamics needs rollout depth before a control can be chance-level. Sweep tooling: `scripts/probes/nca_state_prediction_sweep.py` (k × delta_scale grid).
- Campaign test budget: state-prediction certifies at 100 epochs (bar 0.81, margin ~0.16); the recorded operating point (300ep) is the default.
...
- [x] **Sequence-level ψ statistics — REDEEMED 2026-09-13** (see the A/B session below): the temporal ψ law already carries decayed sufficient statistics (gram/cross) in the live ψ dict across episodes/tasks; the A/B comparison campaign (final-step vs per-timestep stepping) was run and the variant shipped as an opt-in `psi_step` knob.
...
- **Remaining TODO23:** PyPI publishing only (external: credentials + sibling packages first). Sequence-ψ statistics redeemed the same day (A/B session below).
...
**Remaining TODO23:** PyPI publishing only (external — credentials + publish sibling packages first). All in-repo items complete as of the NCA state-prediction session.
