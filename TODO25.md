...
7. **T25.B.3 (H24.3) is instrument-blocked, not compute-blocked.** The continual corpus path requires a geometry-backed system; the flagship ψ mechanism is a bare ridge readout. The unblocking work is a **composed backbone+ψ system** (trainable feature-extractor θ + ψ ridge on top): ψ arm = frozen θ with per-episode ridge re-solve, θ-finetune control = SGD on the same backbone at matched compute, frozen_no_psi = frozen θ + linear probe. Scoped as the first TODO26 item; round 1's ψ-underperformance result should carry an "instrument ≠ registered benchmark" caveat wherever cited.
