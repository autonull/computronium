...
**Status:** In progress — Phase A + X1–X3 + Phase B code complete; Phase C: C2/C3/C5/C7 done, C1 dep-only, C4/C6 remaining; D2 core (pareto/load) done, D/X4–X5 remaining.
...
**Done (checkpoint: D2 core — pareto vectorization + KB loader uncaps):**
- **D2-core ✅ `pareto_top` rewritten** (`computronium/visualization/atlas.py`): O(n²) Python double loop (16.3 s @5k) → `_dominated_2d` O(n log n) sort-sweep for 2-objective NaN-free frames + `_dominated_vectorized` chunked numpy broadcast otherwise (memory-bounded via `_DOMINANCE_CHUNK`, preserves original NaN-as-equal + strict-domination + duplicate-survive semantics). **Measured: 5k×2 = 4.8 ms; 5k×5 = 406 ms.** Verified vs brute-force reference: 8 randomized trials (n=1…400, d=2/3/5, NaN 0–10%, dup 0–100%, mixed max/min via oriented sign flip) all OK + full-cost/default membership + sort-order + k-head + empty/missing-column passthrough. Side effect: `pareto_top` complexity lint finding (14>10) gone.
- **D2-core ✅ KB loader 100-row cap fixed** (correctness bug, not just perf): `KnowledgeBase.query()` defaults `limit=100`, so `load_cells` and `_load_measured_cells` silently truncated every campaign >100 cells (health tiles, coverage, strata, front history, graveyard, costs, maturation, atlas all wrong on real roots). Both now `query(limit=UNBOUNDED_ROWS)` (new sentinel in atlas.py) + `auto_embed=False` (no vector-store init on read paths) + missing-file guard in `load_cells`.
- **Perf after fixes (probe, 5k cells):** `load_cells` 159 ms, `_with_layout` 311 ms, `render_snapshot(no atlas)` 2235 ms, `adapt_discovery_map` ~1021 ms (was 16.9 s). Remaining hotspots = repeated KB loads (front_history ×1 + every helper re-queries ~100 ms each), iterrows in `_with_layout` + `create_discovery_map_from_atlas`, and `DashboardApp._get_panel_data` rebuilding a full snapshot per panel key.
- **Verification (this checkpoint):** dev-env smoke OK; `ruff format --check` clean on touched files; `ruff check` on the two files: 3 findings remain, **all pre-existing on HEAD** (Register C: `noqa-comments` broad_map:530, `complex-structure` promote_candidates/apply_bp_deficit — my pareto rewrite removed the 4th); `pyright` 5 errors in broad_map all pre-existing on HEAD (stash-verified), atlas.py clean; targeted suites **21 passed** (UX-L1 ×6, UX-L12, UX-L15 ×4, dashboard smoke) + **12 passed** (`tests/ui/`).

**Handoff notes for next session (research done — implement, don't re-explore):**
1. **User directive:** no brittle tests — C6 pixel-baseline plan row is **superseded** (see C6 below). All new locks behavioral, generous thresholds.
2. **D2 remaining hotspots → implementation:** (a) mtime-keyed cache for `load_cells`/`_load_measured_cells` keyed `(path, mtime_ns, size)` returning `df.copy(deep=False)` / `list(rows)` — pandas 3.0.5 CoW + frozen `KnowledgeEntry`/`_CellRow` verified safe for this; (b) vectorize `_with_layout` (kill `df.iterrows`, 311 ms); (c) `create_discovery_map_from_atlas` → `df.to_dict("records")`; (d) `DashboardApp` caches ONE `render_snapshot` per refresh cycle (`_clear_panel_data` invalidates); (e) **delete dead `render_snapshot(...)` call in `_refresh_cheap`** (computed, result discarded). Then `tests/perf/test_budgets.py` (NOT in pyproject `testpaths` — invoke by path): DiscoveryMap adapter ≤100 ms @5k (median-of-3, warm), `render_snapshot` warm budget, `WS_PAINT_INTERVAL ≥1.0 s` (already true: `_WS_PAINT_INTERVAL_S = 2.0`), row-virtualization cap.
3. **Row virtualization:** `DiscoveryMap._render_table` (unbounded rows → `ui.table`) and `RepairBench.render` loop are the >1k-row paths; cap at `MAX_RENDERED_ROWS = 1000` + "showing first N" glossary key (must pass UX-L4 FK≤8; `tr()` = glossary lookup, 256 terms, no cap key yet). ActivityFeed (30) / FieldReports (20) / VetoLog ([:50]) already capped. `ui.table` exposes `.rows` for assertions; `ui.element` has NO `.attrs` method (use `.props`/`.classes`).
4. **C4 axe (environment fully probed):** chromedriver 153 + chromium 153 installed at `/usr/bin` ✓; **selenium NOT installed → `uv add --dev selenium`**; vendor `axe.min.js` from `/home/me/mcr/node_modules/axe-core/axe.min.js` (v4.10.3, 553 KB) into `tests/a11y/fixtures/`; **C1:** drop nonexistent `nicegui[testing]` extra (pyproject lines 95 + 129). `nicegui.testing` module exists (Screen/User/…) but has **no pytest11 entry point** → register via top-level `conftest.py` with `pytest_plugins = ["nicegui.testing.plugin"]` (pytest8 forbids `pytest_plugins` in non-root conftest). Screen fixture: `helpers.is_pytest()` checks `PYTEST_CURRENT_TEST`; `ui.run` then honors `NICEGUI_SCREEN_TEST_PORT`. Keyboard-crawl manual skip stays.
5. **C6 UX-L6 (revised, non-brittle):** (a) static lock — every `OutcomeBadge` style has a unique (icon, label) pair (shape/label redundancy = grayscale-proof by construction; `_OUTCOME_STYLES` in live_atlas: 9 badges); (b) one behavioral Screen screenshot pair (populated vs empty fixture root) → PIL grayscale → mean-abs-diff above a generous floor (states distinguishable without color). **No committed pixel baselines** (supersedes "store under tests/ui/baselines/"). `REDUCED_MOTION_CSS` exists and is already token-tested in UX-L5.
6. **AdapterContext formalization (smell #4):** change `DataAdapter.adapt` to take `AdapterContext(root, snapshot, recognition_store=None)`; `FunctionAdapter` unpacks `(ctx.snapshot, ctx.root)`; progress registered via context-aware adapter reading `ctx.recognition_store`; `DashboardApp._get_panel_data` builds one context, **delete the `functools.partial` special-case (dashboard.py:451-459) and its `partial` import (line 11)**. Only `.adapt(` call site is dashboard.py:460; UX-L15 tests call plain `adapt_*` fns directly → unaffected.
7. **log-path bug (found):** new `ui.dashboard` NEVER calls `resolve_log_path` — CLI default `log_path=None` → ticker permanently empty. Fix in `DashboardApp.__init__`: `self.log_path = resolve_log_path(root, log_path)` (+ re-resolve on root switch). Old `live_atlas.build_dashboard` (line 1510) does it right; smoke tests import THAT one — two `build_dashboard`s coexist (CLI uses `ui.dashboard`'s) — unify later (opportunity).
8. **X4 multi-root:** CLI `--root` type=str, split `,` → `tuple[Path, ...]`; `DashboardApp(roots=..., root=roots[0])`; header `ui.select` when `len(roots)>1`; `switch_root(path)` resets EmbedCache/signature/objectives/pareto_state/recognition_store (per-root `ui_state.sqlite`)/`_panels`/`_panel_data` + `_register_root_panels(path)` + re-render. `ArtifactChanged` already carries `root` — handler should drop foreign roots.
9. **X5 config hot-reload:** in `_poll`, also stat `root/campaign.yaml` + `root/heartbeat.json`; on change re-parse objectives (`_objectives_from_heartbeat` + campaign.yaml `hpo.objectives` — sample `experiments/campaign_gate_tier0_digits.yaml`; **`epoch_time_s` is NOT in `Objective` enum → `parse_objectives` raises ValueError → wrap, fall back to current**). Publish `ConfigChanged(objectives=...)` (event exists in event_bus) → subscriber updates `pareto_state`, clears panel data, re-renders.
10. **D3 rebuild flag is a NO-OP today:** `DashboardApp.__init__` calls `store.rebuild_from_events()` and **discards the result; `persist_state` never called**. Fix: `persist_state(rebuild_from_events())` when `rebuild_state=True`. e2e test: `append_event` kinds that yield badges — `pareto_front_expanded` → `gold_standard`, `measurement_recorded`+`is_void` → `honest_broker`, `cell_completed`×3 → quest progress — then assert `get_last_rebuild_at()` + non-empty badges/quests + `_get_panel_data("progress")` → `ProgressPanel.update_data` round-trip. CLI `--help` flag already asserted in `test_ux_l8:136`.
11. **D1 observability (no heavy deps):** `computronium/ui/metrics.py` — `threading.Lock` registry, counters + reservoir histograms → `render_prometheus()` text; route via `app.get("/metrics")` (nicegui App extends FastAPI ✓); instrument `_render_current_panel` / `_get_panel_data` / `_on_ws_event`. structlog + OpenTelemetry = **deferred opportunity** (heavy deps; stdlib `logging` + `extra=` already in use — note deviation in D1 row when closing).
12. **D4:** `docs/platform/dashboard.md` (flags incl. comma-list roots, modes, 20-panel table, kill switches: `--ui-actions` default off / `COMPUTRONIUM_GAMIFY` / `--no-open`, registry→adapter→bus diagram, extension how-to: register + adapter + optional bus sub) + README link near the `docs/platform/` sentence (~line 534) and/or `comp dashboard` row.
13. **D5:** gallery lock EXPECTED derives from `DEMOS` — no dashboard DEMOS row exists → **verify-no-op** unless a dashboard demo is registered; run `tests/integration/test_gallery_lock.py` + `test_gallery_lock_declared.py` to confirm.
14. **Repo hygiene facts:** ruff 0.16.6(preview) **flags `# noqa:`** ("noqa-comments") — suppressions must be `# ruff: ignore[rule]`; ~half the repo is mid-migration. **35 pre-existing dirty files in worktree are NOT mine** (continual/hyperopt/deployments/ceec/scripts/tests/docs run_records + partial noqa migrations in ui/a11y, progress_panel, recognition/*) — **stage only files you edited**. `run_records/*.json` dirty from an earlier gallery run — don't stage. `websockets` imported by dashboard WS consumers but absent from pyproject (verify transitive via nicegui before relying). `tests/{ui,lint,visual,a11y,integration}` not in `testpaths` — always pass explicit paths. `HAS_SENTENCE_TRANSFORMERS=False`, no FAISS (KB init prints warnings per construction — loader cache also cuts these).
