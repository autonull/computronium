# ruff: file-ignore[unsorted-imports]
# ruff: file-ignore[implicit-namespace-package]
"""SparseEqprop algorithm.

Sparse Equilibrium Propagation with dynamic sparsity

Reference implementation:
    computronium.algorithms.sparse_eqprop.reference

Accelerated kernel:
    computronium.algorithms.sparse_eqprop.kernel
"""

# Register the spec when this module is imported
from computronium.acceleration.registry import register as _register

from .cases import make_case
from .factory import create_sparse_eqprop_mlp
from .kernel import step as kernel_step
from .reference import step as reference_step
from .spec import SPEC

_register(SPEC)  # ruff: ignore[non-empty-init-module]

__all__ = [
    "SPEC",
    "create_sparse_eqprop_mlp",
    "kernel_step",
    "make_case",
    "reference_step",
]
