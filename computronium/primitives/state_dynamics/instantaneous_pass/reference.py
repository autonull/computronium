"""Reference implementation for Instantaneous Pass.

Delegates to computronium.ontology.dynamics.InstantaneousDynamics (the source of truth).
...
def _case_to_composite_state(case: Any) -> CompositeState:
    """Convert a test case to a CompositeState for InstantaneousDynamics."""
    # InstantaneousDynamics expects a CompositeState with x (input) and layer activations
...
def step(case: Any) -> Any:
    ...
    execution.
    """
    config = StateDynamicsConfig.instantaneous(
        max_steps=case.config["steps"],
        step_size=case.config.get("step_size", 0.1),
    )
    dynamics = InstantaneousDynamics(config)
