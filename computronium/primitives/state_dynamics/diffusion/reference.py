"""Reference implementation for Diffusion.

Delegates to computronium.ontology.dynamics.DiffusionDynamics (the source of truth).
...
from computronium.ontology.dynamics import DiffusionDynamics, StateDynamicsConfig
...
def _case_to_composite_state(case: Any) -> CompositeState:
    """Convert a test case to a CompositeState for DiffusionDynamics."""
    # DiffusionDynamics expects a CompositeState with x (input) and layer activations
...
def step(case: Any) -> Any:
    ...
    execution.
    """
    config = StateDynamicsConfig.diffusion(
        max_steps=case.config["steps"],
        step_size=case.config.get("step_size", 0.1),
    )
    dynamics = DiffusionDynamics(config)
