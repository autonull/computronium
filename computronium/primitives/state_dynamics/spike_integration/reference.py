"""Reference implementation for Spike Integration.

Delegates to computronium.ontology.dynamics.SpikeIntegrationDynamics (the source of truth).
...
def _case_to_composite_state(case: Any) -> CompositeState:
    """Convert a test case to a CompositeState for SpikeIntegrationDynamics."""
    # SpikeIntegrationDynamics expects a CompositeState with x (input) and layer activations
...
def step(case: Any) -> Any:
    ...
    execution.
    """
    config = StateDynamicsConfig.spike_integration(
        max_steps=case.config["steps"],
        step_size=case.config.get("step_size", 0.1),
    )
    dynamics = SpikeIntegrationDynamics(config)
