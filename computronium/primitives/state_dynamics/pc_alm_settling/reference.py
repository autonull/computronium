...
def _case_to_composite_state(case: Any) -> CompositeState:
    """Convert a test case to a CompositeState for PCALMDynamics."""
    # PCALMDynamics expects a CompositeState with x (input) and layer activations
    state = CompositeState(activity={"x": case.state}, plastic={}, substrate={})
    # Store predictions and multipliers in metrics for the dynamics to use
    state.metrics = {
