"""Reference implementation for Muon (Riemannian Orthogonal Update).

Delegates to computronium.ontology.update.RiemannianOrthogonalUpdate (the source of truth).
...
def step(case: Any) -> dict[str, torch.Tensor]:
    ...
    """
    config = ParameterUpdateConfig.muon(
        step_size=case.config.get("step_size", 0.01),
        momentum=case.config.get("momentum", 0.9),
        ortho_steps=case.config.get("ortho_steps", 0),
    )
    update = RiemannianOrthogonalUpdate(config)
