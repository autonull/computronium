"""Reference implementation for Random Projections Credit.

Delegates to computronium.ontology.credit.RandomProjectionsCredit (the source of truth).
...
def step(case: Any) -> list[torch.Tensor]:
    ...
    """
    config = CreditAssignmentConfig.random_projections(
        feedback_scale=case.config.get("feedback_scale", 0.1),
    )
    ...
    # Simple MSE loss against a target
