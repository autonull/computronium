"""Deterministic test cases for Target Inversion.

These cases are intentionally small. They are used for parity tests,
smoke tests, and microbenchmarks. They are not scientific benchmarks.
"""

from dataclasses import dataclass
from typing import Any

import torch

from computronium.ontology.geometry import FeedforwardGeometry, GeometryConfig


@dataclass(frozen=True, slots=True)
class Case:
    state: torch.Tensor
    prediction: torch.Tensor
    multiplier: torch.Tensor
    activations: list[torch.Tensor]
    nudged_activations: list[torch.Tensor]
    config: dict[str, Any]
    geometry: FeedforwardGeometry


def make_case(
    *,
    device: str = "cpu",
    dtype: torch.dtype = torch.float32,
    seed: int = 0,
) -> Case:
    generator = torch.Generator(device=device).manual_seed(seed)

    # Input batch of 2, feature dim 4
    state = torch.randn(2, 4, device=device, dtype=dtype, generator=generator)
    prediction = torch.randn(2, 4, device=device, dtype=dtype, generator=generator)
    multiplier = torch.randn(2, 4, device=device, dtype=dtype, generator=generator)

    # Create a fixed geometry for deterministic testing
    # Save/restore RNG state to ensure deterministic geometry creation
    rng_state = torch.get_rng_state()
    torch.manual_seed(seed)
    try:
        geometry = FeedforwardGeometry(
            GeometryConfig.feedforward(input_dim=4, output_dim=4, hidden_dims=(8,))
        )
    finally:
        torch.set_rng_state(rng_state)

    # Create dummy activations for free and nudged phases
    # [input, hidden, output]
    activations = [
        state,
        torch.randn(2, 8, device=device, dtype=dtype, generator=generator),
        torch.randn(2, 4, device=device, dtype=dtype, generator=generator),
    ]
    nudged_activations = [
        state,
        torch.randn(2, 8, device=device, dtype=dtype, generator=generator),
        torch.randn(2, 4, device=device, dtype=dtype, generator=generator),
    ]

    config = {
        "beta": 0.5,
        "target": 1,  # Class index for one-hot target
        "seed": seed,
    }

    return Case(
        state=state,
        prediction=prediction,
        multiplier=multiplier,
        activations=activations,
        nudged_activations=nudged_activations,
        config=config,
        geometry=geometry,
    )
